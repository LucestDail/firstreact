...
export default function ({
  ...
  currentBoxId = id;
  const { getNode, getNodes, setNodes, setEdges, addNodes } = useReactFlow();
  const clickItem = useCallback((itemType, event) => {
    let currentSelectedNode = getNode(currentBoxId);
    const newItem = NewItemOption(currentSelectedNode.id, currentSelectedNode.itemNumber, itemType);
    setNodes(
      getNodes().map((node) => {
  ...
  return (
    <>
      <div
        ...
        {...props}
      >
        <div className="Right" style={{ width: '100%', height: '100%', background: 'white', borderRadius: 4, border: '1px #E0E0E0 solid', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center', display: 'inline-flex' }}>
